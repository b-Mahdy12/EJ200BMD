...
G4VPhysicalVolume *DetectorConstruction::Construct()
{
  ...
  WLSFiber_up_Logical = new G4LogicalVolume(wire, WLS, "upSingleWireLogical");
  WLSFiber_down_Logical = new G4LogicalVolume(wire, WLS, "downSingleWireLogical");
  ...
  fSipm_side1_Logical = new G4LogicalVolume(SiPM, silicon, "SiPMLogical_top");
  fSipm_side2_Logical = new G4LogicalVolume(SiPM, silicon, "SiPMLogical_bottom");
  ...
  {
    XYCenter += 1 * cm;
    G4VPhysicalVolume *wire_up_Phy = new G4PVPlacement(rotb, G4ThreeVector(XYCenter, 60 * cm, 0.05 * cm), WLSFiber_up_Logical, "WLS_up", fScintillatorLogical, false, k + 1, checkOverlaps);
    G4VPhysicalVolume *wire_down_Phy = new G4PVPlacement(0, G4ThreeVector(0 * cm, XYCenter, 1.7 * cm), WLSFiber_down_Logical, "WLS_down", fScintillatorLogical, false, k + 1, checkOverlaps);
  }
  G4VPhysicalVolume *Sipm_topSide1_Phy = new G4PVPlacement(0, G4ThreeVector(59.9 * cm, 0 * cm, -0.35 * cm), fSipm_side1_Logical, "SiPM_S1_up", WLSFiber_up_Logical, false, 0, checkOverlaps);
  G4VPhysicalVolume *Sipm_topSide2_Phy = new G4PVPlacement(0, G4ThreeVector(-0.1 * cm, 0 * cm, -0.35 * cm), fSipm_side2_Logical, "SiPM_S2_up", WLSFiber_up_Logical, false, 0, checkOverlaps);

  G4VPhysicalVolume *Sipm_downSide1_Phy = new G4PVPlacement(0, G4ThreeVector(59.9 * cm, 0 * cm, -0.35 * cm), fSipm_side1_Logical, "SiPM_S1_down", WLSFiber_down_Logical, false, 0, checkOverlaps);
  G4VPhysicalVolume *Sipm_downSide2_Phy = new G4PVPlacement(0, G4ThreeVector(-0.1 * cm, 0 * cm, -0.35 * cm), fSipm_side2_Logical, "SiPM_S2_down", WLSFiber_down_Logical, false, 0, checkOverlaps);

  // G4VPhysicalVolume *physScint2 = new G4PVPlacement(0, G4ThreeVector(-25. * cm, -25 * cm, 0. + 2. * cm), fScintillatorLogical, "ScintillatorPhysical", logicWorld, false, 0, false);

  // G4VPhysicalVolume *Sipm_bottomSide1_Phy = new G4PVPlacement(0, G4ThreeVector(60 * cm, 0 * cm, 0.35 * cm), fSipm_side1_Logical, "SiPM_S2_down", WLSFiber_down_Logical, false, 0, checkOverlaps);
  // G4VPhysicalVolume *Sipm_bottomSide2_Phy = new G4PVPlacement(0, G4ThreeVector(0 * cm, 0 * cm, 0.35 * cm), fSipm_side2_Logical, "SiPM_S2_down", WLSFiber_down_Logical, false, 0, checkOverlaps);
  ...
  fSipm_side1_Logical->SetVisAttributes(cyancol);
  fSipm_side2_Logical->SetVisAttributes(greencol);
  WLSFiber_up_Logical->SetVisAttributes(orangecol);
  WLSFiber_down_Logical->SetVisAttributes(yellowcol);
...
void DetectorConstruction::ConstructSDandField()
{
  ...
  //
  SenDet *WLS_up = new SenDet(SDname = "Detector/WLS_up");
  G4SDManager::GetSDMpointer()->AddNewDetector(WLS_up);
  WLSFiber_up_Logical->SetSensitiveDetector(WLS_up);

  SenDet *WLS_down = new SenDet(SDname = "Detector/WLS_down");
  G4SDManager::GetSDMpointer()->AddNewDetector(WLS_down);
  WLSFiber_down_Logical->SetSensitiveDetector(WLS_down);

  //
  // SiPM
  //
  SenDet *SiPM_top = new SenDet(SDname = "/Detector/SiPM_top");
  G4SDManager::GetSDMpointer()->AddNewDetector(SiPM_top);
  fSipm_side1_Logical->SetSensitiveDetector(SiPM_top);

  SenDet *SiPM_bottom = new SenDet(SDname = "/Detector/SiPM_bottom");
  G4SDManager::GetSDMpointer()->AddNewDetector(SiPM_bottom);
  fSipm_side2_Logical->SetSensitiveDetector(SiPM_bottom);

  // GetScoringVolume()->SetSensitiveDetector(SDlogicalvolume);
}
