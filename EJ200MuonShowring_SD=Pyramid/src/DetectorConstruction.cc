...
*/
  ...
  // Pyramid
  //
  auto meshPy = CADMesh::TessellatedMesh::FromSTL("Pyramid_of_Khafre.stl");
  meshPy->SetScale(10.0);
  // mesh->SetOffset(G4ThreeVector(-0.5 * scintillatorSizeX, 0, -0.5 * scintillatorSizeX));
  // mesh->SetOffset(G4ThreeVector(0,0,0));
  auto Khafre_py = meshPy->GetSolid();
...
/*
  ...
  G4VPhysicalVolume *physScint1 = new G4PVPlacement(0, G4ThreeVector(-25. * cm, -25 * cm, 0. * cm), fScintillatorLogical, "ScintillatorPhysical", Khafrelog, false, 0, false);
  G4VPhysicalVolume *physScint2 = new G4PVPlacement(0, G4ThreeVector(-25. * cm, -25 * cm, 0. + 2. * cm), fScintillatorLogical, "ScintillatorPhysical", Khafrelog, false, 0, false);
